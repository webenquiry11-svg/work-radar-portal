    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm">
        <div className="p-6 border-b">
          <h3 className="text-lg font-semibold text-slate-800">Add Holiday</h3>
          <p className="text-sm text-slate-500">{date.toLocaleDateString('en-US', { dateStyle: 'full' })}</p>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            ...
            <input
              ...
              className="w-full text-sm border border-slate-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm">
        <div className="p-6 text-center">
          <div className="mx-auto bg-red-100 rounded-full h-12 w-12 flex items-center justify-center my-4"><ExclamationTriangleIcon className="h-6 w-6 text-red-600" /></div>
          <h3 className="text-lg font-semibold text-slate-800">Confirm Deletion</h3>
          <p className="text-sm text-slate-500 mt-2">Are you sure you want to remove "{holiday.name}" as a holiday?</p>
        </div>
...
const HolidayManagement = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-full flex flex-col">
      ...
      `}</style>
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Holiday Management</h1>
        <p className="text-slate-500 mt-2">Manage your company's holiday calendar. Click on a date to add or remove a holiday.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-6 flex flex-col">
          <h3 className="text-lg font-semibold text-slate-800 mb-4 border-b pb-3">Upcoming Holidays</h3>
          <div className="flex-1 overflow-y-auto">
            {upcomingHolidays.length > 0 ? (
              <ul className="space-y-3">
                {upcomingHolidays.map(holiday => (
                  <li key={holiday._id} className="flex items-center justify-between p-3 rounded-lg bg-red-50">
                    <div>
                      <p className="font-semibold text-red-800">{holiday.name}</p>
                      <p className="text-xs text-red-600">{new Date(holiday.date).toLocaleDateString('en-US', { dateStyle: 'long', timeZone: 'UTC' })}</p>
                    </div>
                    <button onClick={() => setDeletingHoliday(holiday)} className="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-100 rounded-full"><TrashIcon className="h-4 w-4" /></button>
                  </li>
                ))}
              </ul>
            ) : (
