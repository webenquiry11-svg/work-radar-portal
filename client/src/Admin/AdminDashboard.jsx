    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm">
        <div className="p-6 text-center">
          ...
          <p className="text-sm text-slate-500 mt-2">Are you sure you want to permanently delete the report from <strong className="text-slate-700">{new Date(report.reportDate).toLocaleDateString()}</strong>? This action cannot be undone.</p>
        </div>
  ...
  return (
    <header className="relative z-50 h-16 bg-white/80 backdrop-blur-lg border-b border-gray-200 flex items-center justify-between px-4 sm:px-6 flex-shrink-0 shadow">
      ...
      <div className="flex items-center gap-4">
        <button onClick={handleRefresh} className="text-indigo-500 hover:text-indigo-700 p-2 rounded-full hover:bg-indigo-50" title="Refresh Data">
          <span className="sr-only">Refresh data</span>
          <ArrowPathIcon className="h-6 w-6" />
        </button>
        <div className="w-px h-6 bg-gray-200"></div>
        <div className="relative group">
          <button onClick={handleBellClick} className="text-indigo-500 hover:text-indigo-700 p-2 rounded-full hover:bg-indigo-50 relative">
...
export default function AdminPageLayout() {
  ...
  return (
    <CurrentUserProvider>
      <div className="flex h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 font-manrope">
        <style>
          {`
