  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-full flex flex-col bg-slate-50/50 dark:bg-slate-900 font-manrope">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-extrabold text-slate-800 dark:text-slate-200 tracking-tight">All Employee Attendance</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-2">View attendance records for all employees in the organization.</p>
      </div>
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-4 gap-6 md:gap-8">
        <div className="xl:col-span-1 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg flex flex-col">
          <div className="p-4 border-b dark:border-slate-700">
            <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Employees ({filteredEmployees.length})</h2>
            <div className="relative mt-3">
              ...
              <input
                ...
                className="pl-10 w-full text-sm border-slate-300 dark:border-slate-600 rounded-lg p-2 focus:ring-blue-500 bg-white dark:bg-slate-700 dark:text-white"
              />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {filteredEmployees.map(employee => (
              <button
                ...
                className={`w-full text-left p-3 my-1 rounded-lg transition-all flex items-center gap-3 ${selectedEmployee?._id === employee._id ? 'bg-blue-100 dark:bg-blue-900/50' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}
              >
                ...
                <div>
                  <p className={`font-semibold ${selectedEmployee?._id === employee._id ? 'text-blue-800 dark:text-blue-300' : 'text-slate-800 dark:text-slate-200'}`}>{employee.name}</p>
